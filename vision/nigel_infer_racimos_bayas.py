...
random.seed(5)
output =      '4_estado/'   # Si la carpeta no existe, se crea
...
class GrapesDetector:
    ...
    def detect_grapes(self):
        ...
        filtered_masks = []
        image_grapes_masks = []
        dataset = os.path.basename(self.input_images)
        modelo = os.path.splitext(os.path.basename(self.model_path))[0]
        fecha = datetime.now().strftime('%b%d').lower()
        ...
        # df.to_csv('output/area_racimos-bayas_0614_sep1_th01.csv')
        df.to_csv(f'{output}area_racimos-bayas_{dataset}_{fecha}_th01.csv')
        print('Terminó de graficar')


def main():
    # input_path = 'input/dataset/0720'
    input_path = 'input/dataset/4_estado'
    model_grapes = 'modelos/model_v4.pth'
