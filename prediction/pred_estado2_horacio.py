...
name = 'AUG_area_racimos-bayas_0627_sep1_th01.csv'
df = pd.read_csv(name)

#############################################
#Eliminar filas con 0 en horacio
df = df[(df != 0).all(1)]
##guardar para verificar que no existan filas con 0
df.to_csv('Nozero_'+name)
#############################################
#print(f'data: {data}')
...
#plt.show()
plt.savefig('estado2_horacio.png')

##evaluacion en dataset test
torch.save(model, 'estado2_horacio.pth')
model.eval()
y_pred = model(X_test)
row_list = []
