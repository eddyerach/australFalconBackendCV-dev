...
class BunchDetector:
    ...
    def detect_bunches(self):
        ...
        filtered_masks = {}
        print(f'Procesando imagen: ', self.input_path)
        ...
        if filtered_masks.keys():
            ...
            output_masks.append([self.input_path,black_image,final_contours,biggest_mask,biggest_contour])
...
class GrapesDetector:
    ...
    def detect_grapes(self):
        ...
        for idx, image in enumerate(self.input_masks):
            ...
            print(f'self.input_images: {self.input_images}, image[0]: {image[0]}')
            org_image = cv2.imread(self.input_images)
            ...
            res_det = len(grapes_masks)
            idx_comp_sol    = area_bayas_imagen/image[3]
            idx_comp_no_sol = not_solaped_contour/image[3]
            print(f'det: {res_det}, idx_comp_sol: {idx_comp_sol}, idx_comp_no_sol: {idx_comp_no_sol}')
        ...
        #df.to_csv('area_racimos-bayas.csv')
        #print(f'res: {filtered_masks}')
        return res_det, idx_comp_sol, idx_comp_no_sol
